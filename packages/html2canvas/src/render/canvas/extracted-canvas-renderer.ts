...
import {TextBounds} from '../../css/layout/text-bounds';
...
import {CHECKBOX, INPUT_COLOR, RADIO} from '../../dom/replaced-elements/input-element-constants';
import {TEXT_ALIGN} from '../../css/property-descriptors/text-align';
import {TextShadow} from '../../css/property-descriptors/text-shadow';
import {PAINT_ORDER_LAYER} from '../../css/property-descriptors/paint-order';
import {Renderer} from '../renderer';
import {DIRECTION} from '../../css/property-descriptors/direction';
import type {FontMetric} from '../font-metrics';
import {segmentGraphemes} from '../../css/layout/text-segmentation';
...
export interface CanvasRendererEnvironment {
...
export class ExtractedCanvasRenderer extends Renderer {
  canvas: HTMLCanvasElement;
  ctx: CanvasRenderingContext2D;
  private readonly _activeEffects: IElementEffect[] = [];
  private readonly fontMetrics: FontMetricsProvider;
  private readonly environment: CanvasRendererEnvironment;

  constructor(context: RenderContext, options: RenderConfigurations, environment: CanvasRendererEnvironment) {
  ...
  private createCanvasElement(ownerDocument?: Document | null): HTMLCanvasElement {
...
const isTextInputElement = (container: ElementContainer): container is TextInputContainer => {
...
type ImageContainer = ReplacedElementContainer & {
  containerType: 'image';
  src: string;
...
const isImageContainer = (container: ElementContainer): container is ImageContainer =>
  container.containerType === 'image';

const isInputContainer = (container: ElementContainer): container is InputContainer =>
...
const fixIOSSystemFonts = (fontFamilies: string[], userAgent: string): string[] => {
  return /iPhone OS 15_(0|1)/.test(userAgent)
    ? fontFamilies.filter((fontFamily) => iOSBrokenFonts.indexOf(fontFamily) === -1)
    : fontFamilies;
};

const hasCanvasStyle = (
  canvas: HTMLCanvasElement
): canvas is HTMLCanvasElement & {style: {width: string; height: string}} =>
