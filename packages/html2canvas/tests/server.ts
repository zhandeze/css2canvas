...
const comparePngBuffers = (
		...
		for (let x = 0; x < overlapWidth; x += 1) {
			...
			if (same) {
				const gray = Math.max(
					...
					Math.min(
						255,
						Math.round(referencePixel.r * 0.3 + referencePixel.g * 0.59 + referencePixel.b * 0.11)
					)
				);
	...
	const compare: ImageCompareResult = {
		identical:
			diffPixels === 0 && referencePng.width === actualPng.width && referencePng.height === actualPng.height,
		diffPixels,
