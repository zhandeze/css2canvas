...
const applyDefaultJsonContentType = (
    req: express.Request,
...
const defaultExampleServerPort = Number(process.env.MINIAPP_EXAMPLE_SERVER_PORT || process.env.HTML2CANVAS_EXAMPLE_PORT || 8080);
...
const comparePngBuffers = (referenceBuffer: Buffer, actualBuffer: Buffer): {compare: ImageCompareResult; diffBuffer: Buffer} => {
    ...
    const markDiffPixel = (x: number, y: number, referencePixel?: {r: number; g: number; b: number; a: number}, actualPixel?: {r: number; g: number; b: number; a: number}) => {
            ...
            if (same) {
                const gray = Math.max(
                    ...
                    Math.min(
                        255,
                        Math.round(
                            referencePixel.r * 0.3 + referencePixel.g * 0.59 + referencePixel.b * 0.11
                        )
                    )
                );
